...
export function CmsPage({ page }: { page: NonNullable<ReturnType<typeof getSitePage>> }) {
  return (
    <div>
      <section className="bg-ford text-white">
        <div className="mx-auto max-w-6xl px-4 py-10 md:py-14">
          ...
          <h1 className="mt-2 max-w-3xl text-3xl font-semibold tracking-tight md:text-4xl">{page.heading}</h1>
          {page.description ? <p className="mt-3 max-w-2xl text-sm text-white/80">{page.description}</p> : null}
        </div>
      </section>
        ...
        <article className="space-y-5">
          ...
          {page.inventoryHref ? (
            <Link href={page.inventoryHref} className="inline-flex h-11 items-center bg-primary px-5 text-sm font-medium text-primary-foreground hover:bg-[#002654]">
              Shop matching inventory
            </Link>
          ) : null}
          <p className="text-xs text-muted-foreground">
            Sales {dealership.phones.sales} · {dealership.showroom.address}, {dealership.showroom.city}
          </p>
        </article>
